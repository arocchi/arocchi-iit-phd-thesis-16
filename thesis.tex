...
In particular, in the field of humanoid robotics, \todo{how many teams built their own robot, how many used Atlas?} the DRC saw the entry of a number of robots, with entries built from the ground up including the hardaware, while others only focusing on the software development for algorithms in the fields of low and high level control, planning, sensing and artificial intelligence, as well as teleoperation interfaces, transmission on degraded channels for teleoperation and sensing, task specification languages, automated testing infrastractures, control frameworks, and simulation techniques for fast and robust simulation.
