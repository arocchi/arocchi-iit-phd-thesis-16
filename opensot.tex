...
\subsubsection{Cartesian, CoM, MinimumCoMVelocity, MiminumCartesianVelocity}
...
\subsubsection{BilateralConstraint}
...
\end{equation}
A similar idea was presented also in \cite{Park:98} but contact forces were not taken in consideration.
...
\section{Weighted Pseudo-Inverse}
...
\subsection{Integration with Robotic Frameworks}
...
\subsection{Integration with Generic Simulators}
...
\subsubsection{Resources Allocation: Velocity Allocation}
...
\keywords{Inverse Kinematics, Quadratic Programming Optimization, Dynamics.}
...
In \eqref{eq:sot_full}, priorities are taken into account considering the previous solutions $\dq_i, \; i < n$ and constraints of the type {$\mathbf{A}_i \dq = \mathbf{A}_i \dq_i, \; \forall i < n$},  so that the optimality of all higher priority tasks is not changed by the current solution \cite{mansard2009versatile}. The second term in the cost function of \eqref{eq:sot_full} permits to handle kinematics singularities in order to avoid high joint velocities \cite{Nakamura1990ARR533662}. A similar structure can be used to solve the IK problem at the acceleration level.
...
A similar idea was presented also in \cite{Park:98} but contact forces were not taken in consideration, while they are fundamental when working with floating base robots.
