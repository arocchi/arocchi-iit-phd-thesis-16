...
The \emph{YCM} framework developed by the iCub facility department has been used for the WALK-MAN project, which allows to automatically manage the build order, the automatic downloading, compilation and installation of dependencies and the management of versions for each separate module in the build, and integrates seamlessly with the ROS build system, either \emph{rosbuild} or \emph{catkin}.
...
With these concepts in mind we decided to extend one of the most known robotics simulator, Gazebo (\cite{koenig2004design}), to be compatible with one of the most used robotics framework, YARP, developed at the Italian Institute of Technology. 
...
Controlling a new robot with \textbf{OpenSoT} requires only writing a new stack (in most cases, a pre-made stack can be used for common whole-body tasks). Thanks to \nameref{sec:idynutils}, every robot which is provided with an \emph{URDF} and \emph{SRDF} can be imported for use with the library.
